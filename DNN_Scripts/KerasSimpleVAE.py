...
def vae_loss(input_img, output):
    ...
    kl_loss = - 0.5 * K.sum(1 + z_log_sigma - K.square(z_mean) - K.exp(z_log_sigma))
...
autoencoder.compile(optimizer=keras.optimizers.Adam(), loss=vae_loss)
...
with open('architectureVAE.json', 'w') as arch_file:
    arch_file.write(arch)

    autoencoder.save("modelVAE_arch.h5")
    autoencoder.save_weights("modelVAE_weights.h5")
...
mydict+="}"                     
output_pkl = open('variablesVAE.pkl', 'wb')
